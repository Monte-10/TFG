...
function CreateDayOption() {
  ...
  const [nutritionTotals, setNutritionTotals] = useState({ calories: 0, protein: 0, carbohydrates: 0, fat: 0, sugar: 0, fiber: 0, saturated_fat: 0, gluten_free: true, lactose_free: true, vegan: true, vegetarian: true, pescetarian: true, contains_meat: false, contains_vegetables: false, contains_fish_shellfish_canned_preserved: false, cereal: false, pasta_or_rice: false, dairy_yogurt_cheese: false, fruit: false, nuts: false, legume: false, sauce_or_condiment: false, deli_meat: false, bread_or_toast: false, egg: false, special_drink_or_supplement: false, tuber: false, other: false});
  ...
  const handleSubmit = async (event) => {
    ...
    // Asegurándonos de que authToken esté disponible.
    ...
    try {
      ...
      if (!response.ok) {
        // Si la respuesta no es ok, lanzamos un error para capturarlo más adelante
      ...
      // Limpieza o acciones adicionales después de la creación exitosa
      // Por ejemplo: resetear formulario, mostrar mensaje de éxito, redireccionar, etc.

    } catch (error) {
      console.error('Error creating option:', error);
      setError('Failed to create option. Please try again.');
      // Aquí puedes manejar el estado de error, por ejemplo, mostrando un mensaje de error en la UI
    }
};
        ...
        {!optionCreated ? (
            <form onSubmit={handleSubmit}>
                <div className="mb-3">
                    ...
                    <input type="text" className="form-control" id="optionName" value={name} onChange={e => setName(e.target.value)} placeholder="Option Name" />
                </div>

                <div className="mb-3">
                    <label htmlFor="breakfastSelect" className="form-label">Breakfast:</label>
                    <select className="form-select" id="breakfastSelect" value={selectedMeals.breakfast} onChange={(e) => handleMealChange('breakfast', e.target.value)}>
                ...
                <div className="mb-3">
                    ...
                    <select className="form-select" id="midMorningSelect" value={selectedMeals.mid_morning} onChange={(e) => handleMealChange('mid_morning', e.target.value)}>
                ...
                <div className="mb-3">
                    ...
                    <select className="form-select" id="lunchSelect" value={selectedMeals.lunch} onChange={(e) => handleMealChange('lunch', e.target.value)}>
                ...
                <div className="mb-3">
                    ...
                    <select className="form-select" id="snackSelect" value={selectedMeals.snack} onChange={(e) => handleMealChange('snack', e.target.value)}>
                ...
                <div className="mb-3">
                    ...
                    <select className="form-select" id="dinnerSelect" value={selectedMeals.dinner} onChange={(e) => handleMealChange('dinner', e.target.value)}>
                ...
                <div className="mb-3">
                    ...
                    <select multiple className="form-select" id="extrasSelect" value={selectedMeals.extras} onChange={handleExtrasChange}>
                        {meals.map((meal) => (
                            <option key={meal.id} value={meal.id}>{meal.name}</option>
                        ))}
                    </select>
